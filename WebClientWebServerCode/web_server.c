...
int main() {
    ...
    int signal_fd[2];
    if(pipe(signal_fd) < 0) {
        fprintf(stderr, "Failed to create signal pipe.\n");
        return 1;
    }
    ...
    // while (g_running) {
    //     fd_set reads = wait_on_clients(&client_list, server, signal_fd[0]); // Pass the signal file descriptor
    //     // ... (existing code for the main loop)
    // }
    ...
    while(g_running) {
        ...
        fd_set reads = wait_on_clients(&client_list, server, signal_fd[0]);
